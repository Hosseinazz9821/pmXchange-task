...
import { getRialForBuy, getUSDTForBuy } from "../requests";
...
export default function Buy() {
  ...
  useEffect(() => {
    if (requestType === "RIAL") {
      const getData = async () => {
        setLoader(true);
        await getUSDTForBuy(rialAmount, setUsdAmount);
        setLoader(false);
      };
  ...
  useEffect(() => {
    if (requestType === "USDT") {
      const getData = async () => {
        setLoader(true);
        await getRialForBuy(usdAmount, setRialAmount);
        setLoader(false);
      };
  ...
  return (
    <div className="Sell_content_Container">
      ...
      <div className="Button_content">
        <button className="StyleButton_content">خرید</button>
      </div>
    </div>
  );
}
